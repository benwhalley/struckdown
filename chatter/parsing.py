...
mindframe_grammar = (Path(__file__).parent / "chatter.lark").read_text()
...
class MindframeTransformer(Transformer):
    ...
    def placeholder(self, items):
        self.current_buf.append({"type": "variable", "name": ".".join(map(str, items[0]))})
        return None
...
def extract_placeholders(template_text):
    ...
    try:
        ...
        user_input_placeholders = [p for p in all_placeholders if p not in completion_keys]
